# 순회자 (Iterator)
## 객체의 구성 요소들을 쉽게 순회할 수 있도록 해주는 도구
## for 루프를 통한 반복이 가능
...
class LinkedListBasicPlus:
    ...
    def insert(self, i:int, newItem):  
        if i >= 0 and i <= self.__numItems: 
            ...
            # Plus 과제부분
            if prev == None:   #번호를 찾아서 이전 노드가 비어있으면
                self.__head = ListNode(newItem, None)   #self.head에 아이템과 꼬리를 연결해서 추가함 (이전노드)
                prev = self.__head  #이전노드로 연결
            newNode = ListNode(newItem, prev.next)
            prev.next = newNode     #newNode에 추가하려는 아이템 연결
    ...
    def append(self, newItem):
        ...
        if prev == None:   #번호를 찾아서 이전 노드가 비어있으면
            self.__head = ListNode(newItem, None)   #self.head에 아이템과 꼬리를 연결해서 추가함 (이전노드)
            prev = self.__head   #이전노드로 연결
        newNode = ListNode(newItem, prev.next)   #newNode에 추가하려는 아이템 연결
        prev.next = newNode 
        self.__numItems += 1
